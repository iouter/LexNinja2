...
public class HengheShui() : LexNinja2Card(1,
    CardType.Skill, CardRarity.Rare,
    TargetType.AnyAlly)
{
    protected override IEnumerable<DynamicVar> CanonicalVars => [new HealVar(15),new PowerVar<PoisonPower>(3)];
        ...
        CardPlay play)
    {
        ...
        int poisonPoint = Owner.RunState.Rng.CombatEnergyCosts.NextInt(DynamicVars.Power<PoisonPower>().IntValue);
        if (play.Target != null)
        {
            await CreatureCmd.Heal(play.Target,healPoint,true);
            await PowerCmd.Apply<PoisonPower>(choiceContext, play.Target, poisonPoint,Owner.Creature,this);
