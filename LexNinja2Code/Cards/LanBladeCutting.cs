...
public class LanBladeCutting() : LexNinja2Card(1,
    CardType.Power, CardRarity.Uncommon,
    TargetType.Self)
{
    protected override IEnumerable<DynamicVar> CanonicalVars => [new NinjutsuVar(1)];
    protected override IEnumerable<IHoverTip> ExtraHoverTips => [ HoverTipFactory.FromCard<LanBlade>(IsUpgraded)];
        ...
        CardPlay play)
    {
        ...
        if (IsUpgraded)
        {
            await PowerCmd.Apply<LanBladePowerUpgraded>(new ThrowingPlayerChoiceContext(), Owner.Creature, 1, Owner.Creature, this);
            if (Ninjutsu())
            {
        ...
        await PowerCmd.Apply<LanBladePower>(new ThrowingPlayerChoiceContext(), Owner.Creature, 1, Owner.Creature, this);
        if (Ninjutsu())
        {
            CardModel card = CombatState.CreateCard<LanBlade>(Owner);
            await CardPileCmd.AddGeneratedCardToCombat(card,PileType.Hand,Owner);
