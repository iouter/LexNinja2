...
public class LeechFriend() : LexNinja2Card(2,
    ...
    TargetType.AnyEnemy)
{
        ...
        CardPlay play)
    {
        if (play.Target.GetPower<WeakPower>() != null)
        {
            ...
            await PowerCmd.Apply<StrengthPower>(new ThrowingPlayerChoiceContext(), play.Target, -DynamicVars.Strength.BaseValue, Owner.Creature, this);
            await PowerCmd.Apply<DexterityPower>(new ThrowingPlayerChoiceContext(), play.Target, -DynamicVars.Strength.BaseValue, Owner.Creature, this);
        }
    }
