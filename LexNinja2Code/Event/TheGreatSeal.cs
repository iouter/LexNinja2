...
{
    public override string? CustomInitialPortraitPath => "res://LexNinja2/images/events/TheGreatSeal.png";
    ...
    private async Task SaveHamood()
    {
        ...
        CardCmd.PreviewCardPileAdd(await CardPileCmd.Add(Owner.RunState.CreateCard<Normality>(base.Owner), PileType.Deck));
        for (int i = 0; i < 2; i++)
        {
            await RelicCmd.Obtain(RelicFactory.PullNextRelicFromFront(base.Owner,RelicRarity.Rare).ToMutable(), base.Owner);
        }
    ...
    private async Task HamoodKillAll()
    {
        ...
        CardCmd.PreviewCardPileAdd(await CardPileCmd.Add(Owner.RunState.CreateCard<HamoodKillAll>(base.Owner), PileType.Deck));
